...
TimeStamp Decoder::pts2TimeStamp(int64_t pts, AVRational *pts_timebase) {
	return (TimeStamp)(pts*pts_timebase->num/(double)pts_timebase->den*FOBS_TIMESTAMP_UNITS_D);
   }
...
ReturnCode Decoder::_open()
{
   ...
   */
   ...
   {
       if(inputFile->streams[audioStreamIndex]->codec->channels > 2)
       {
...
ReturnCode Decoder::reallocTransitionPicture(int newWidth, int newHeight)
{
      ...
      transitionPictureHeight != newHeight)
   {
      ...
      if(isOk(error))
      {
         img_resample_ctx = img_resample_init( newWidth, newHeight, 
                             this->_getWidth(), this->_getHeight());
     ...
     else if(pkt->size > 0) 
     {
        if(videoFrame)
        {
            ...
            else
            {
               ...
               //cout << "PosNew=" << position << "   DTS="<<pkt->dts<<"   PTS=" << pkt->pts << "  DEN=" << vDen << "   NUM=" << vTimeBase.num << endl;
        ...
        else
        {
            ...
            if(pkt->dts == (TimeStamp)AV_NOPTS_VALUE)
            {
                ...
                //cout << "FigureingOut Audio Position: "<< positionAudio << endl;
            ...
            else
            {
			   ...
			   //cout<<"PosAudioDTS="<<positionAudio<<endl;
            }
        }
     }
     else
...
ReturnCode Decoder::decodeAudioFrame()
{
   ...
   if(isOk(error))
   {
    ...
    else
    {
        ...
        if(audioResampler)
        {
            static uint8_t resampled[AVCODEC_MAX_AUDIO_FRAME_SIZE];
            //Resampling needed
            int nSamples = data_size / inputFile->streams[audioStreamIndex]->codec.channels/2;
            audio_resample(audioResampler, 
                                  (short *)resampled, (short *)samples,
                                  nSamples);
