...
ReturnCode Transcoder::transcode()
{
  ...
  {
    e->setVideoParameters(vWidth, vHeight, vBitRate, vFrameRate, vCodec);
  }
  else
  {
    e->setVideoParameters(d);
  }
  if(customAudioSettings)
  {
    e->setAudioParameters(aSamplesPerSec, aChannels, aBitRate, aCodec);
  }
  else
  {
    e->setAudioParameters(d);
  }
  
  e->open();
  TimeStamp duration = d->getDurationMilliseconds();
