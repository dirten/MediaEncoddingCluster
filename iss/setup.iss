...
AppVerName=Media Encoding Cluster 0.0.4.7
...
OutputBaseFilename=MediaEncodingCluster-Setup-0.0.4.7
...
;Filename: "{app}/erts-/bin/erl.exe" ; Parameters: "-config releases/0.0.4.7/sys -eval ""setup:setup_windows_service(),init:stop()."" "; WorkingDir: "{app}"
;Filename: "{app}/erts-/bin/erl.exe" ; Parameters: "-setcookie default -config releases/0.0.4.7/sys -eval ""setup:setup_win32(client,yes),init:stop()."" "; WorkingDir: "{app}"; Tasks: baseclient and not baseserver
;Filename: "{app}/erts-/bin/erl.exe" ; Parameters: "-setcookie default -config releases/0.0.4.7/sys -eval ""setup:setup_win32(both,yes),init:stop()."" "; WorkingDir: "{app}"; Tasks: baseserver and baseclient
...
[Files]
;Source: "/releases/0.0.4.7/tmp/*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs
...
[Icons]
;Name: "{group}\start Runtime"; Filename: "{app}\erts-\bin\erl.exe" ; Parameters: "-setcookie default -config releases/0.0.4.7/sys -name 'console' -eval ""application:start(sasl), user_default:startserver(), init:stop()."" "; WorkingDir: "{app}"
;Name: "{group}\stop Runtime"; Filename: "{app}\erts-\bin\erl.exe" ; Parameters: "-setcookie default -config releases/0.0.4.7/sys -name 'console' -eval ""application:start(sasl), user_default:stopserver(), init:stop()."" "; WorkingDir: "{app}"
...
;Name: "{group}\start Console"; Filename: "{app}\erts-\bin\werl.exe" ; Parameters: "-sname mhive_console -setcookie default -config releases/0.0.4.7/sys"; WorkingDir: "{app}"
