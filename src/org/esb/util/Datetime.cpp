...
namespace util{
  ...
  inline unsigned short getNumber2(const char* s)
  {
    if (!isdigit(s[0])
      || !isdigit(s[1]))
      throw IllegalFormatException("Datetime::getNumber2");
    return (s[0] - '0') * 10
         + (s[1] - '0');
  }
  ...
  inline unsigned short getNumber3(const char* s)
  {
    if (!isdigit(s[0])
      || !isdigit(s[1])
      || !isdigit(s[2]))
      throw IllegalFormatException("Datetime::getNumber3");
    return (s[0] - '0') * 100
         + (s[1] - '0') * 10
         + (s[2] - '0');
  }
  ...
  inline unsigned short getNumber4(const char* s)
  {
    if (!isdigit(s[0])
      ...
      || !isdigit(s[2])
      || !isdigit(s[3]))
      throw IllegalFormatException("Datetime::getNumber4");
  ...
  Datetime Datetime::fromIso(const std::string& s)
  {
    if (s.size() < 23
      ...
      || s.at(16) != ':'
      || s.at(19) != '.')
      throw IllegalFormatException("Datetime::fromIso");
