...
const litesql::FieldType OutputFile::Outfiledata("outfiledata_","BLOB",table__);
...
OutputFile::OutputFile(const litesql::Database& db, const litesql::Record& rec)
    ...
    case 7: status = convert<const std::string&, std::string>(rec[6]);
        status.setModified(false);
    case 6: outfiledata = convert<const std::string&, litesql::Blob>(rec[5]);
...
std::vector<litesql::Database::SchemaItem> HiveDb::getSchema() const {
    ...
    res.push_back(Database::SchemaItem("OutputFile_","table","CREATE TABLE OutputFile_ (id_ " + backend->getRowIDType() + ",type_ TEXT,filename_ TEXT,path_ TEXT,jobid_ TEXT,outfiledata_ BLOB,status_ TEXT)"));
