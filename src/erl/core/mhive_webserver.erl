...
-export([start_link/1,terminate/2, init/1,code_change/3,handle_call/3,handle_cast/2,handle_info/2]).
...
init([Mode, Port|Rest])->
    io:format("Webserver start with args~w~n",[Mode]),
    process_flag(trap_exit, true),
    inets:start(),
    filelib:ensure_dir(getConfig(error_log)),
    StartPage=case Mode of
                  setup->"setup.html";
                  _->"mec.html"
              end,
    Conf=[
