%%%----------------------------------------------------------------------
%%% File    : packet_server.erl
%%% Author  : Jan Hölscher <jan.hoelscher@esblab.com>
%%% Purpose : Sends and receives the Packets to and from the Encoding Clients
...
get_job()->
    JobFun = fun() ->
            Q = qlc:q([{Jobs#job.id, filename:join(Files#file.path,Files#file.filename), Jobs#job.last_ts, Jobs#job.pass} || Jobs <- mnesia:table(job),Files <- mnesia:table(file),Jobs#job.status==active,Jobs#job.complete_time == undefined,Jobs#job.infile==Files#file.id ]),
            C = qlc:cursor(Q),
