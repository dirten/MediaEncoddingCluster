...
create()->
    mnesia:create_table(packet, [{type, set}, {disc_only_copies,[node()]}, {attributes, record_info(fields, packet)}]).
...
read5(JobId)->
    ...
    E.
import(Count)->
    case gen_server:call({global,packet_sender}, {packetstream,"/media/disk/video/big_buck_bunny_1080p_surround.avi",-1,"0",500  },infinity)of
    %  case gen_server:call({global,packet_sender}, {packetstream,"/media/TREKSTOR/videos/ChocolateFactory.ts",-1,-1,1000  })of
      ...
      %-record(packet,{id,pts, dts,data, data_size}).
            io:format("Data:"),
            Result=case mnesia:transaction(
                fun()->
                            [
                                mnesia:write(#packet{
                                id=libdb:sequence(packet),
                                pts=element(3,X),
                                data=element(10,X)})||X<-Any, X/=eof
                            ]
                end) of
                {atomic,A}->A;
                {aborted,_}->io:format("Any Data:~p",[Any])
            end,
...
export(cursor)->
    F = fun() ->
            Q = qlc:q([[PU] || PU <- qlc:keysort(1,mnesia:table(packet),{order, descending}), element(4,PU)==45]),
            C = qlc:cursor(Q),
            qlc:next_answers(C, 1)
        end,
    {atomic,E}=mnesia:transaction(F),
