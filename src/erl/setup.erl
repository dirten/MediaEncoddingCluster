...
setup_win32(Mode, AutoStart)->
  ...
  case AutoStart of
    yes->
      ...
      os:cmd(lists:concat([ErtsPath,"/bin/erlsrv add MHiveService -w ",code:root_dir()," -name ",Node," -d console -args \"-mnesia dir 'data' -setcookie default -config releases/"++libutil:toString(?VERSION)++"/sys -boot releases/"++libutil:toString(?VERSION)++"/start\""]));
    _->
      ok
  end.
