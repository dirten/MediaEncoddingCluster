...
namespace encodingtask {
  ...
  boost::shared_ptr<org::esb::hive::job::ProcessUnit> StreamProcessUnitBuilder::build(org::esb::av::PacketListPtr list, Ptr<org::esb::av::Decoder> dec, Ptr<org::esb::av::Encoder> enc) {
    ...
    //int cou = u->_gop_size;
    u->_frameRateCompensateBase = frameRateCompensateBase;
    if (u->_decoder->getCodecType() == AVMEDIA_TYPE_VIDEO) {
      ...
      frameRateCompensateBase = f / pow((double) 10, dec.getExponent()*-1);
    }
    if (u->_decoder->getCodecType() == AVMEDIA_TYPE_AUDIO) {
      ...
      raw_out_samples += frameRateCompensateBase;
