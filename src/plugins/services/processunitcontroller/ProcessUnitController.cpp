...
namespace org {
  namespace esb {
    namespace plugin {
      ...
      void ProcessUnitController::startService() {
        LOGDEBUG("start request");
        _stop_signal = false;
        _isRunning = false;
        boost::thread t(boost::bind(&ProcessUnitController::start, this));
      ...
      void ProcessUnitController::stopService() {
        LOGDEBUG("stop request");
        stop();
        LOGDEBUG("stopped");
      ...
      boost::shared_ptr<org::esb::hive::job::ProcessUnit> ProcessUnitController::getAudioProcessUnit() {
        boost::mutex::scoped_lock scoped_lock(get_pu_mutex);
        boost::shared_ptr<org::esb::hive::job::ProcessUnit> u;
