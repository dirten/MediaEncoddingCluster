...
namespace partitionservice {
      ...
      void PartitionManager::putProcessUnit(std::string partition, boost::shared_ptr<org::esb::hive::job::ProcessUnit>unit, Type type) {
        ...
        LOGDEBUG("Putting unitid"<<unit->_process_unit<<" to queue "<<stream_index);
        _streams[stream_index].count++;
        _stream_queues[stream_index]->enqueue(unit);
        LOGDEBUG("Queue Size:"<<_stream_queues[stream_index]->size());
        /*
        std::string data;
